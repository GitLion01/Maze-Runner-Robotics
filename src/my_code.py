...
class PathFollower:
    def __init__(self):
        ...
        self.distance_threshold = 3  # meters
    ...
    def odom_callback(self, msg):
        ...
        self.current_x, self.current_y= self.calculate_real_world_position(msg.pose.pose.position.x+0.75,msg.pose.pose.position.y-0.75)
    ...
    def get_angle_to_goal(self, goal_x, goal_y):
        ...
        goal_angle = math.atan2(dy, dx)  # Note the negative !!!
        angle_diff = goal_angle - self.current_yaw
        #rospy.loginfo(f"goal_y : {goal_y}  , current_y: {self.current_y}")   
        ...
        while angle_diff < -math.pi:
            angle_diff += 2 * math.pi
        '''
        angle_diff_2 = (angle_diff + math.pi) % (2 * math.pi) - math.pi
        
        #rospy.loginfo(f"yaw : {self.current_yaw}, goal_angle: {goal_angle}, ang_diff : {angle_diff}, {angle_diff_2}")
        
        return angle_diff_2

    def calculate_real_world_position(self,x,y):

        rospy.loginfo(f"from the func: {x}, {y}")
        resolution = 0.05
        origin = [-99.9, -100.18]
        
        grid_x = int(( (x) - origin[0]) / resolution)  # Calculate X pixel
        grid_y = int(( (y) - origin[1]) / resolution)  # Calculate Y pixel
    ...
    def pixel_to_real(self, pixel_x, pixel_y):
        ...
        """
        real_x = (pixel_x * 0.05) + -99.9
        real_y = (pixel_y * 0.05) + -100.18
    ...
    def move_to_goal(self):
        ...
        goal_x, goal_y = self.path[self.current_goal_index]
        real_x, real_y = self.pixel_to_real(goal_x,goal_y)
        current_x, current_y = self.pixel_to_real(self.current_x,self.current_y)
        rospy.loginfo(f"goal_x:{goal_x}, goal_y:{goal_y}, goal_x: {real_x}, goal_y {real_y}")
        rospy.loginfo(f"x:{self.current_x}, y:{self.current_y}, goal_x: {current_x}, goal_y {current_y}")
